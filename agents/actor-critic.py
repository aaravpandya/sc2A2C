...
class A2CAtari(base_agent.BaseAgent):
    ...
    def __init__(self,
        ...
        self.action_policy_x = []
        self.action_policy_y = []
        # build network
    ...
    def step(self, obs):
        ...
        screen_features = preprocess_screen(screen_features)
        minimap_features = preprocess_minimap(minimap_features)
    ...
    def _sample_action(self,
        ...
        for arg_type in arg_types:
            if len(arg_type.sizes) > 1:
                ...
                # x_args = x_policy.argsort()[::-1]
                
                x_ids = np.arange(len(x_policy))
                x = np.random.choice(x_ids, p=x_policy)

                y_policy = np.squeeze(y_policy)
                # y_args = y_policy.argsort()[::-1]
                # self.action_policy_x.append(x_policy[x_args])
                # self.action_policy_y.append(y_policy[y_args])
    ...
    def _handle_episode_end(self):
        ...
        #     writer.writerows(self.action_policy_x)
        # with open('y.csv', 'w') as csvFile:
        #     writer = csv.writer(csvFile)
        #     writer.writerows(self.action_policy_y)
        self.counter=self.counter+1
        self.learning_rate=self.learning_rate*(1 - 0.9 * self.counter / int(1e+5))
